...
correctArray(actual == prediction) = 1;
...
% Assuming PR prediction = -1
TN = sum(correctArray(actual == -1));
N = numel(actual(actual == -1));

if (N+P) ~= numel(actual)
    disp('Error: Actual vector contains invalid elements (Elements other than 1 or -1')
end
